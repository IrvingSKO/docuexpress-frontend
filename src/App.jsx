...
function Toast({ toast, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", bottom: 20, right: 20, zIndex: 9999 }}>
      ...
      >
        <b>{toast.title}</b>
        <div style={{ fontSize: 13, marginTop: 6 }}>{toast.message}</div>
...
export default function App() {
  ...
  useEffect(() => {
    if (!BACKEND_URL || BACKEND_URL.includes("undefined")) {
      console.error("VITE_BACKEND_URL inválida:", BACKEND_URL);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    authFetch("/api/users").then(async (r) =>
      setUsers(await safeJson(r))
    );
    authFetch("/api/logs").then(async (r) =>
      setLogs(await safeJson(r))
    );
  }, [me]);
  ...
  const download = async (id, name) => {
    const res = await fetch(`${BACKEND_URL}/api/download/${id}`, {
      headers: {
